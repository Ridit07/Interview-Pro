...
st.title('LipSync Studio') 
options = os.listdir('/Users/riditjain/Downloads/LIPINC-main/data/s1')
...
if options: 
    ...
    with col2: 
        ...
        # Verify the shape of yhat and adjust sequence length if necessary
        sequence_length = yhat.shape[1]  # dynamically get the sequence length based on yhat
        st.write("Shape of yhat:", yhat.shape)

        # Decode using the calculated sequence length
        decoder = tf.keras.backend.ctc_decode(yhat, [sequence_length], greedy=True)[0][0].numpy()

        # Convert the decoded output to text
